    ...
    <div className="fixed bottom-0 left-0 right-0 z-50">
      ...
      <div className="relative bg-gradient-to-t from-gray-900 via-gray-800 to-transparent">
        ...
        <div className="absolute top-0 left-0 right-0 h-20">
          <div 
            className={`
              absolute top-2 h-16 w-24 rounded-2xl
              bg-gradient-to-r ${tabs[morphPosition]?.gradient}
              shadow-2xl transition-all duration-500 ease-out
              opacity-90
            `}
            style={{
              left: `calc(${morphPosition * 33.333}% + ${morphPosition * 8}px + 16px)`,
          ...
          <div 
            className={`
              absolute top-4 h-12 w-20 rounded-xl
              bg-gradient-to-r ${tabs[morphPosition]?.gradient}
        ...
        {/* Navigation Items */}
        <div className="relative flex items-end justify-around px-4 pt-4 pb-8">
          {tabs.map((tab, index) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`
                  ...
                  w-20 h-16 rounded-2xl overflow-hidden
                  transition-all duration-300 ease-out
                  ${isActive ? 'transform -translate-y-2 scale-110' : 'hover:scale-105'}
                  group
                `}
              >
                {/* Ripple Effects */}

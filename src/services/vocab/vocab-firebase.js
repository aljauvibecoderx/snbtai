...
export const saveVocab = async (userId, vocabData) => {
  try {
    ...
    const docRef = await addDoc(vocabRef, {
      ...
      subtest: vocabData.subtest || 'lit_ing', // 'lit_ing' | 'tps_ppu' | 'lit_ind'
...
// Get vocab list for user (with optional subtest filter)
export const getVocabList = async (userId, subtestFilter = null) => {
  try {
    const vocabRef = collection(db, 'vocab');
    let q;
    
    if (subtestFilter && subtestFilter !== 'all') {
      q = query(
        vocabRef, 
        where('userId', '==', userId),
        where('subtest', '==', subtestFilter),
        orderBy('savedAt', 'desc')
      );
    } else {
      q = query(vocabRef, where('userId', '==', userId), orderBy('savedAt', 'desc'));
    }
...
// Get vocab stats (with optional subtest filter)
export const getVocabStats = async (userId, subtestFilter = null) => {
  try {
    const vocabList = await getVocabList(userId, subtestFilter);
    const now = new Date();

...
import { HighlightPopup } from '../vocab/VocabMode';
import { saveVocab, checkVocabExists, getVocabByWord } from '../../services/vocab/vocab-firebase';
...
export const DetailSoalView = ({ questions, subtestLabel, subtestId, onBack, user, questionSetId, showToast }) => {
  ...
  const isVocabEnabled = subtestId === 'lit_ing' || subtestId === 'tps_ppu' || subtestId === 'lit_ind';
  ...
  // Handle text selection for vocab
  const handleTextSelection = async (question) => {
    const currentSubtest = question?.subtest || question?.subject || question?.subtestName || subtestId || 'lit_ing';
    const isQuestionVocabEnabled = currentSubtest === 'lit_ing' || currentSubtest === 'tps_ppu' || currentSubtest === 'lit_ind';
    
    if (!isQuestionVocabEnabled || !user) return;
  ...
  const handleSaveVocab = async (vocabData) => {
    if (!user) {
      addNotification('Login untuk menyimpan vocab', 'warning');
      return;
    }
    ...
    try {
      ...
      if (exists) {
        addNotification('Kata sudah ada di vocab list', 'info');
        return;
      }
      ...
      addNotification('Vocab berhasil disimpan! +5 XP', 'success');
      setHighlightPopup(null);
    } catch (error) {
      console.error('Error saving vocab:', error);
      addNotification('Gagal menyimpan vocab', 'error');
    }
  };
          ...
          {questions.map((q, idx) => {
            ...
            <div key={q.id} id={`question-${idx}`} className="bg-white rounded-xl border border-slate-200 p-6 transition-all">
              ...
              <div 
                ...
                onMouseUp={() => handleTextSelection(q)}

...
import { Book, Search, X, Bookmark, TrendingUp, Award, Flame, Globe, BookOpen, BookMarked } from 'lucide-react';
import { saveVocab, getVocabList, updateVocabReview, getVocabStats, deleteVocab, subscribeToVocabList } from '../../services/vocab/vocab-firebase';

const SUBTEST_FILTERS = [
  { id: 'all', label: 'All', icon: Book, color: 'violet' },
  { id: 'lit_ing', label: 'EN', icon: Globe, color: 'violet' },
  { id: 'tps_ppu', label: 'PPU', icon: BookOpen, color: 'violet' },
  { id: 'lit_ind', label: 'LBI', icon: BookMarked, color: 'violet' }
];

// Vocab Panel (Desktop Only - Sidebar) - UPDATED with subtest tabs
export const VocabPanel = ({ isOpen, onClose, userId, onSearchClick, currentSubtest = 'lit_ing' }) => {
  ...
  const loadStats = async () => {
    try {
      const data = await getVocabStats(userId, activeTab === 'all' ? null : activeTab);
      setStats(data);
    } catch (error) {
  ...
  return (
    <div className="fixed right-2 sm:right-4 top-20 sm:top-24 w-56 sm:w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-3 sm:p-4 space-y-3 sm:space-y-4 z-40 max-h-[80vh] overflow-y-auto">
      ...
      <div className="pb-2 sm:pb-3 border-b border-slate-200">
        ...
        {/* Subtest Tabs - Compact */}
        <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
          {SUBTEST_FILTERS.slice(1).map((filter) => {
      ...
      <div className="space-y-2 sm:space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-base sm:text-lg font-bold text-violet-600">{stats.total}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[10px] sm:text-xs text-slate-600">Total XP</span>
          <span className="text-base sm:text-lg font-bold text-purple-600">{stats.xp}</span>
        </div>
      </div>
...
// Highlight Popup (Desktop & Mobile) - UPDATED with subtest support
export const HighlightPopup = ({ word, x, y, onSave, onClose, existingVocab, subtest = 'lit_ing' }) => {
  const [meaning, setMeaning] = useState(existingVocab?.meaning || '');
  const [loading, setLoading] = useState(false);
  const [showInput, setShowInput] = useState(!existingVocab);

  const subtestLabels = {
    lit_ing: { label: 'Inggris', color: 'blue' },
    tps_ppu: { label: 'PPU', color: 'emerald' },
    lit_ind: { label: 'LBI', color: 'amber' }
  };

  const currentSubtest = subtestLabels[subtest] || subtestLabels.lit_ing;
  ...
  return (
    ...
    >
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="text-xs font-bold text-slate-700">"{word}"</div>
          <div className={`px-1.5 py-0.5 rounded text-[9px] font-semibold bg-${currentSubtest.color}-100 text-${currentSubtest.color}-700`}>
            {currentSubtest.label}
          </div>
        </div>
          ...
          <div className="space-y-2">
              <div className="p-3 bg-violet-50 border border-violet-200 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold text-violet-700">Tersimpan</span>
                </div>
          ...
          <>
            {showInput && (
              <input
                ...
                autoFocus
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg mb-2 focus:outline-none focus:ring-2 focus:ring-violet-500"
              />
            )}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-3 py-1.5 text-xs font-medium text-white bg-violet-600 hover:bg-violet-700 rounded-lg transition-colors flex items-center justify-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Bookmark size={12} />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4">
        ...
        {!isSaved && (
          <button
            onClick={onSave}
            className="w-full px-4 py-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2"
          >
            <Bookmark size={16} />
            Simpan ke Vocab
          </button>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {existingVocab && (
            <div className="p-3 bg-violet-50 border border-violet-200 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <Bookmark size={14} className="text-violet-600" fill="currentColor" />
                <span className="text-xs font-bold text-violet-700">Sudah tersimpan</span>
              </div>
        ...
        <button
          ...
          className="w-full px-4 py-3 bg-violet-600 text-white rounded-xl font-medium hover:bg-violet-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Bookmark size={16} />
...
export const VocabReviewReminder = ({ needReview, onStartReview }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-violet-50 to-purple-100 border-2 border-violet-300 rounded-2xl p-6 shadow-lg">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 bg-violet-500 rounded-lg flex items-center justify-center">
          <Flame size={20} className="text-white" />
        </div>
        <div>
          <h3 className="text-lg font-bold text-violet-900">Vocab Review</h3>
          <p className="text-sm text-violet-700">{needReview} kata belum kamu review</p>
        </div>
      </div>
      <button
        onClick={onStartReview}
        className="w-full px-4 py-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl font-bold hover:shadow-lg transition-all"
      >
        Mulai Review
      </button>
    </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
      <div className="flex items-center gap-2 mb-4">
        <Book size={18} className="text-violet-600" />
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-2xl font-bold text-violet-600">{stats.total}</span>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600">Total XP</span>
          <span className="text-2xl font-bold text-purple-600">{stats.xp}</span>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600">Level</span>
          <span className="text-2xl font-bold text-indigo-600">{Math.floor(stats.xp / 100)}</span>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600">Mastered</span>
          <span className="text-2xl font-bold text-violet-600">{stats.mastered || 0}</span>
        </div>
  ...
  if (isFinished) {
    return (
      <div className="bg-white rounded-2xl p-6 shadow-lg text-center space-y-4">
        ...
        <div className="text-3xl font-bold text-purple-600">+{score * 10} XP</div>
        <button
          onClick={onComplete}
          className="w-full px-4 py-3 bg-violet-600 text-white rounded-xl font-medium hover:bg-violet-700 transition-colors"
        >
          Selesai
        </button>
      </div>

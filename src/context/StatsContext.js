...
export const StatsProvider = ({ children, user, myQuestions, coinBalance }) => {
  ...
  const calculateStats = useCallback(async () => {
    console.log('📊 Calculating stats:', { user: !!user, questionsCount: myQuestions?.length, coinBalance });
    ...
    try {
      ...
      if (!bankSoalCacheRef.current || (now - lastFetchRef.current) > CACHE_DURATION) {
        try {
          ...
          console.log('✅ Bank soal fetched:', bankSoal);
          if (bankSoal > 0) {
            bankSoalCacheRef.current = bankSoal;
            lastFetchRef.current = now;
          }
        } catch (err) {
          console.warn('⚠️ Failed to fetch bank soal, using cache:', err);
          bankSoal = bankSoalCacheRef.current || 0;
        }
      } else {
        console.log('📦 Using cached bank soal:', bankSoal);
      }
      ...
      console.log('✅ Stats calculated:', { hariIni, bankSoal, kredit });
